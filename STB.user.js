	...
	function startSTBArmoury() {
		...
		*/
		...
		function scanInventory() {
			...
			jq("#inv_page .item").each(function(index) {
				...
				if(null !== tmp && undefined !== tmp) {
					...
					var name = jq(this).children().children('.name').text();
		...
		function comparaisonFilter(type, level, item) {
			...
			var itemLevel = parseInt(item.attr('level'));
			if(type == '>')
				if(itemLevel > level)
					return true;
				else
		...
		function filterSelect(item) {
			jq(item).each(function() {
				var desc = jq(this).children('.desc').children();
				if(jq(this).attr('cat') != '' && jq(this).css('display') != 'none') {
					desc.children('input').val(desc.children('.setTotal').attr('count'));
				}
			});
		}
		...
		function splitFunction(item, percent) {
			jq(item).each(function() {
				var desc = jq(this).children('.desc').children();
				if(jq(this).attr('cat') != '') {
					desc.children('input').val(Math.round(parseInt(desc.children('.setTotal').attr('count'))*percent/100));
				}
			});
		}
		...
		function importFunction(string) {
			...
			jq.each(array, function(index, value) {
				...
				if(item.length > 0 || simulation) {
					...
					if(diff <= 0 ) {
						item.children('.desc').children().children('input').val(value[2]);
					} else {
						if(simulation) {
							...
							item.children('.desc').children().children('input').val(item.children('.desc').children().children('.setTotal').attr('count'));
						}
					}
				} else {
		...
		function addControls() {
			...
			jq(".item .setTotal").click(function(e) {
				e.preventDefault();
				jq(this).parent().children('input').val(jq(this).attr('count'));
				inputChanged();
			});
